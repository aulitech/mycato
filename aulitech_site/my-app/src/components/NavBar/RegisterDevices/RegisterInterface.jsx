...
const RegisterInterface = ({ user, devices }) => {
  ...
  const handleSave = async () => {
    console.log("Save button clicked. (" + interfaceName + ": " + operationMode + ")");
  ...
  // set default operation mode to pointer
  operationMode || setOperationMode('pointer');
    ...
    <div className="px-4 py-5 sm:p-6 lg:px-8">
      ...
      <div className="mt-5">
        <label htmlFor="interface-name" className="block text-lg text-gray-900">
          Identify the device you will be connecting to
        </label>
        <input
          type="text"
          id="interface-name"
      ...
      <p className="mt-5 text-lg text-gray-900">
        When you click <strong>Save</strong>, your browser may ask if you want to allow access to Cato. You must allow access in order to register the connection.
      </p>
